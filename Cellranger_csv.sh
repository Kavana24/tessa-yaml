...
while IFS=, read heading1 heading2 heading3 heading4 heading5; do
    ...
    echo $PVCSIZE
    java -jar /jenkins-cli.jar -s http://10.60.2.9:8080/ -auth admin:admin build  -p Testing_jenkins_cli size=$PVCSIZE -p id=$id -p transcriptome=$transcriptome -p sample=$sample1 -p fastqs=$fastqs -p gcsbucket=$gcsbucket
    timestamp=$(date +%s)
    echo $timestamp
done
