...
K8JOBNAME="$expidlower"-"seurat3"-"$BUILDID";
podname=`hostname`;
...
then
echo "working"
java -jar /jenkins-cli.jar -s http://10.60.2.9:8080/ -auth admin:admin build Seurat3-success-notification -p jenkinsjobID=$JENKINSJOBNAME-$BUILDID -p k8jobname=$K8JOBNAME -p podname=$podname -p outputgcsbucket=$SEURATOUTGCSBUCKET/$SEURAT3OUTFOLDER -p seurat3outfolder=$SEURAT3OUTFOLDER 
